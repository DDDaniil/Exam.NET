...
namespace Exam.Services
{
    ...
    private int GetScoredFromAdult(int adult, int amount, Deposit depositEnum) =>
        adult switch
        {
            ...
            >= 29 and <= 59 => 14,
            >= 60 and <= 72 => depositEnum == Deposit.None ? 0 : 8,
            _ => 0
        };

    private int GetScoredFromJudging(bool isReallyJudged) => isReallyJudged ? 0 : 15;

    private int GetScoredFromEmployment(Employment employmentEnum, int adult)
    {
    ...
    private int GetScoresFromPurpose(Purpose purposeEnum) =>
        purposeEnum switch
        {
            Purpose.Consumer => 14,
            Purpose.Realty => 8,
    ...
    private int GetScoresFromDeposit(Deposit depositEnum, int carAge) =>
        depositEnum switch
        {
            Deposit.Retiree => 14,
    ...
    private int GetScoresFromOtherCredits(bool otherCredits, Purpose purposeEnum)
    {
        if (otherCredits) return 0;
        return purposeEnum == Purpose.Recrediting ? 0 : 15;
    }
